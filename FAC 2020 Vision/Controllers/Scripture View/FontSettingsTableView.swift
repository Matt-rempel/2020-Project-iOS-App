...
class FontSettingsTableView: UITableViewController {
	...
	// Variable
	var delegate:ScriptureViewController!
	var NUMBER_OF_SECTIONS = 2
	var ROWS_PER_SECTION = [3, 1]
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return ROWS_PER_SECTION[section]
    }
	...
	func showFontSizeActionSheet() {
		...
		for font in fontSizes {
			let action: UIAlertAction = UIAlertAction(title: scriptureSS.getFullFontSize(size: font), style: .default) { action -> Void in
	...
	func showFontStyleActionSheet() {
		...
		for font in fontFamilies {
			let action: UIAlertAction = UIAlertAction(title: scriptureSS.getFullFontName(font: font), style: .default) { action -> Void in
	...
	func showFontColorActionSheet() {
		...
		for color in fontColors {
			let action: UIAlertAction = UIAlertAction(title: scriptureSS.getFullColor(color: color), style: .default) { action -> Void in
	...
	func showScriptureActionSheet() {
		...
		for translation in translations {
			let action: UIAlertAction = UIAlertAction(title: scriptureSS.getFullTranslation(translation: translation), style: .default) { action -> Void in
