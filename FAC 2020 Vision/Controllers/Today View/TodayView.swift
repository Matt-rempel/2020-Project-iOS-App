...
//            print(notifications ?? nil)
            ...
            for notification in notifications ?? [] {
                ...
                if notification.platform == "ALL" || notification.platform == "iOS" {
                    let notificationPage = self.bltnDataSource.getNotification(notification: notification, todayView: self)
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let unsplashLink = cellDevotion.unsplashId

        if let image = UnsplashCache.photos[unsplashLink] {
            ...
            self.usDownloader.getImage(unsplashId: unsplashLink, indexPath: indexPath) { (image, indexPath, error) in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "segue" {
            if let detailVC = segue.destination as? DevotionalDetailView {
                detailVC.devotion = selectedDevotional
            }
        }
    }
}
