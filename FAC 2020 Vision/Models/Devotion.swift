...
//    lazy var scriptureManager = ScriptureManager()
	...
	internal init(json: JSON) {
	    ...
	    unsplashId = json["unsplash_id"].string ?? ""
	    body = json["body"].string
		bodyHTML = json["body_html"].string
	    let authorName = json["author"]["name"].string ?? ""
	    let authorTitle = json["author"]["title"].string ?? ""
		author = Author(title: authorTitle, name: authorName)
        ...
        if scriptureJSONArray.count > 0 {
            for index in 0...scriptureJSONArray.count-1 {
                let scriptureJSON = scriptureJSONArray[index]
                ...
                if index != scriptureJSONArray.count-1 {
                    scriptureList += "\n"
...
//        scriptureManager.getScriptureProgress(devotion: self)
    ...
    internal init() {
        ...
        unsplashId = photos.randomElement() ?? "dqx3HQDrXuw"
